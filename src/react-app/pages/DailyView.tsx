...
import { Calendar, CheckCircle2, Target, TrendingUp, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function DailyView() {
  ...
  const { logs, createOrUpdateLog } = useDailyLogs(format(selectedDate, 'yyyy-MM-dd'));
  ...
  const handleToggleComplete = async (goalId: number, completed: boolean, notes?: string) => {
    try {
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <StatsCard
            ...
            subtitle={completedCount === totalCount && totalCount > 0 ? "Perfect day! 🎉" : "Keep going!"}
            icon={TrendingUp}
            color="blue"
          />
        </div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white/10 backdrop-blur-lg rounded-xl border border-white/20 p-6">
              <h2 className="text-xl font-semibold text-white mb-6">
                {isToday ? "Today's Tasks" : `Tasks for ${format(selectedDate, 'MMMM do')}`}
              </h2>
        ...
        {completedCount === totalCount && totalCount > 0 && (
          <div className="bg-gradient-to-br from-green-500/20 to-emerald-600/20 backdrop-blur-lg rounded-xl border border-white/20 p-8 text-center">
            ...
            <h2 className="text-2xl font-bold text-white mb-2">Perfect Day Achieved!</h2>
            <p className="text-green-200 text-lg mb-4">
              You've completed all your goals for {isToday ? 'today' : format(selectedDate, 'MMMM do')}!
            </p>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-purple-200">
                  ...
                  {totalCount - completedCount === 1 ? ' Just 1 more to go!' : ` ${totalCount - completedCount} more to go!`}
                </p>
              </div>
            </div>
          </div>
